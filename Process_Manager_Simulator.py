    ...
    for _ in range(500):
        ...
        for q in range(1,11):
            ...
            if aw < best_wait:
                best_wait = aw
                best_q = q
        snapshots.append([num_ready,avg_remaining,max_remaining,avg_waiting,cpu_util,best_q])
...
# Streamlit UI
st.title("🖥️ Process Manager Simulator")
...
# Session state
if "process_list" not in st.session_state: st.session_state.process_list=[]
if "cpu_timeline" not in st.session_state: st.session_state.cpu_timeline=[]
if "current_time" not in st.session_state: st.session_state.current_time=0
if "terminated_processes" not in st.session_state: st.session_state.terminated_processes=[]
if "rr_index" not in st.session_state: st.session_state.rr_index=-1
if "deadlock" not in st.session_state: st.session_state.deadlock=False
if "current_quantum" not in st.session_state: st.session_state.current_quantum=0
...
while st.session_state.process_list and not all_terminated:
    ready_queue = [p for p in st.session_state.process_list if p.arrival_time<=st.session_state.current_time and p.state!="Terminated"]
    time_quantum = 0
    if ready_queue:
        if algorithm=="FCFS":
        ...
        st.session_state.current_time+=1
        if any(p.state!="Terminated" for p in st.session_state.process_list):
            st.session_state.deadlock=True
        time.sleep(sim_speed)
    ...
    with placeholder.container():
        ...
        st.subheader("Current Process Table")
        df = pd.DataFrame([{
            "PID":p.pid,
            "State":p.state,
        ...
        st.dataframe(df)
        st.subheader("CPU Timeline (Gantt Chart)")
        ...
        if st.session_state.deadlock:
            st.markdown("⚠️ **Deadlock detected! Some processes are blocked.**")

    all_terminated = all(p.state=="Terminated" for p in st.session_state.process_list)
